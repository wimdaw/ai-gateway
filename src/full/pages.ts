...
export async function renderHomePage(c: Context<{ Bindings: Env }>, isLoggedIn: boolean) {
  ...
  // Host 优先从 URL 解析 (fetch Host 头在部分运行时不可用), 再回退 header, 最后回退默认
...
<div class="admin-shell">
  ...
  <div class="admin-main">
    <header class="admin-topbar">
      ...
      <nav aria-label="移动端控制台导航"><a href="#overview">概览</a><a href="#providers">渠道</a><a href="#proxy-keys">令牌</a></nav>
    ...
    <main class="admin-content">
      ...
      <section id="overview" class="admin-overview" aria-labelledby="admin-title">
        ...
        <div class="admin-metrics" aria-label="配置统计">
          <div><span>${providers.length}</span><p>渠道</p><small>${enabledProvidersCount} 个已启用</small></div>
          <div><span>${modelsCount}</span><p>模型</p><small>${enabledModelsCount} 个可用</small></div>
          <div><span>${proxyKeys.length}</span><p>令牌</p><small>${enabledProxyKeysCount} 个可用</small></div>
          <div><span class="status-dot status-dot--online"><i aria-hidden="true"></i>已配置</span><p>存储</p><small>${storageLabel}</small></div>
        </div>
      </section>
